...
const FadeInSection = memo(({ children, className = "", delay = 0 }: any) => {
  ...
  useEffect(() => {
    ...
    }, { rootMargin: '100px' }); // Renderiza um pouco antes de entrar na tela
  ...
  return (
    <div ref={domRef} className={`${className} transition-all duration-[600ms] ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`} style={{ transitionDelay: `${delay}ms` }}>
      {children}
    </div>
  );
});
...
export default function Catalogo() {
  ...
  // Busca Inteligente com Cache (Evita recálculos pesados)
  const todosTimes = useMemo(() => [...SELECOES_ITEMS, ...BRASILEIRAO_ITEMS], []);
  ...
  // ============================================================================
  // TELA DE SKELETON (Carregamento Premium Inteligente)
  // ============================================================================
  if (loading) return (
